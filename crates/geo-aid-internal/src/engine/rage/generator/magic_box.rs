...
use super::{program::Value, Adjustable, Complex, State};
...
pub fn adjust(current_state: &State, matrix: &mut [Value], adjustment_magnitude: f64) {
    let it = current_state
        .adjustables
        .iter()
        .copied()
    ...
    for ((origin, quality), matrix) in it {
        match origin {
            ...
            Adjustable::Real(val) => {
                ...
                matrix.complex =
                    Complex::real(val + direction * adjustment_magnitude * val * (1.0 - quality));
            }
            Adjustable::Clip1D(val) => {
                ...
                matrix.complex =
                    Complex::real(val + direction * adjustment_magnitude * (1.0 - quality));
            }
        }
    }
}
